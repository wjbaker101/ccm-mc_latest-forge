...
public final class RenderManager {
    ...
    public void drawPartialCircle(
        ...
        for (int i = startAngle; i <= endAngle; ++i) {
            ...
            bufferBuilder
                .pos(x + (float)Math.cos(radians) * radius, y + (float)Math.sin(radians) * radius, 0.0F)
                .color(r, g, b, a)
                .endVertex();
        }
    ...
    public void drawTorus(
        ...
        for (int i = 0; i <= 360; ++i) {
            ...
            bufferBuilder
                .pos(x + (float)Math.cos(radians) * innerRadius, y + (float)Math.sin(radians) * innerRadius, 0.0F)
                .color(r, g, b, a)
                .endVertex();

            bufferBuilder
                .pos(x + (float)Math.cos(radians) * outerRadius, y + (float)Math.sin(radians) * outerRadius, 0.0F)
                .color(r, g, b, a)
                .endVertex();
        }
    ...
    public void drawText(final String text, final int x, final int y, final RGBA colour, final boolean hasShadow) {
        ...
        if (hasShadow)
            Minecraft.getInstance().fontRenderer.drawStringWithShadow(new MatrixStack(), text, x, y, colourAsInt);
        else
            Minecraft.getInstance().fontRenderer.drawString(new MatrixStack(), text, x, y, colourAsInt);
    }
